 ...
 */
    ...
    public int hash(String id){
        ...
        for (int i = 0; i < id.length(); i++) {
            nombre += id.charAt(i)*73;
            
        }
        return nombre % 200 ;
    }
    public Neurotransmisor buscar(String id){
        int indice = this.hash(id);
        return this.hash[indice].buscar(id);
    ...
    public void eliminar(String id){
        int indice = this.hash(id);
        Neurotransmisor e = this.hash[indice].buscar(id);
        this.hash[indice].eliminar(e);
